...
# Raw data and transformations for historical metrics
...
# Retrieve model metadata from risk endpoint
...
if metadata:
    ...
    # Define which metadata to display
    ...
    # Set a default threshold value for later use (and store in session state)
...
with risk_tab:
    ...
    with col_thr:
        # Risk threshold slider (store value in session state so other tabs can access)
        st.session_state.threshold = 100 - st.slider("Model sensibility %", value= 100 - default_threshold, 
                                min_value=0, max_value=100, 
                                key="model_sensibility", help="How sensible is the AI model to the risk detected")
    with col_sample:
        sample_size = st.number_input("Sample size new", value=len(df_sample), min_value=100, max_value=len(df_sample), 
                                          key="sample_size", help="Manage the total sample size sent to the model")
    with col_batch:
        batch_size = st.number_input("Batch size new", value=500, min_value=100, max_value=len(df_sample), 
                                     key="batch_size", help="Manage the data batches sent to the model") 
    st.markdown("---")
    ...
    with col_curr:
        ...
        with col1:
            ...
            st.metric(label="CONTROLED ITEMS [PU]",
        ...
        with col2:
            ...
            control_rate_pre = 1.0
            delta = 0.0  # always 0%
            st.metric(label="CONTROL RATE [%]",
        ...
        with col2:
            ...
            st.metric(label="TAX RAISED RATE [%]",
    ...
    df_with_batch_pred_new['prediction'] = np.where(df_with_batch_pred_new['proba'] >= st.session_state.threshold/100, True, False)
    ...
    with col_plusai:
        ...
        with col1:
            ...
            st.metric(label="CONTROLED ITEMS [PU]",
                    ...
                    delta="{:.2%}".format(delta))
        with col2:
            total_metric(df_sample, df_ai_controls_new, 
                         'illicit', 'ILLICITS', '', 'PU')

        col1, col2 = st.columns(2)
        with col1:
            total_metric(df_sample, df_ai_controls_new, 
                         'RAISED_TAX_AMOUNT_USD', 'TAX RAISED', 'THOUSANDS', 'U$S')
        with col2:
            control_rate_ai = len(df_ai_controls_new)/len(df_sample)
            control_rate_df = len(df_sample)/len(df_sample) #all phisical inspections
