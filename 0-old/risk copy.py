...
# Function to send POST request for model metadata
@st.cache_resource
...
print(metadata)
print(metadata["hist_data_shape"])
print(type(metadata["hist_data_shape"]))
...
### processing with the backend app

## testing backend responde

# Function to send POST request for predictions
@st.cache_resource
...
def predict_batch():
    print('ACA')
    # ddt_new = pd.DataFrame(st.session_state.sample)

    batch_pred_result = send_batch_post_request(st.session_state.sample)

    print(batch_pred_result)
...
def main():
    ...
    ddt_result = ddt_new[['illicit', 'RAISED_TAX_AMOUNT_USD']]

    selection = dataframe_with_selections(ddt_new)

    if st.button('Predect selection'):
        for index, row in selection.iterrows():
            # selected row sending to predict
            data = ddt_new.loc[index].to_dict()
            # sending post request
            prediction_result = send_single_post_request(data)
            print(prediction_result)
            # processing results
            results = {}
            # results['Destinacion e item'] = ddt_new['Destinacion e item'].loc[index]
            ...
            results.update(ddt_new.loc[index].to_dict())            
            print(results)
